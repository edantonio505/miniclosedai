...
| Free disk | ~15 GB build-time space on Docker's `data-root`; final image footprint ~6 GB |
...
- **Two images, two services.** MiniClosedAI (FastAPI app, ~160 MB) and Ollama (extended base + three models baked in as layers, ~5.8 GB) run in separate containers on a shared compose network.
...
| **`Dockerfile.ollama`** | Extends `ollama/ollama:0.5.13`. Bakes each baked model in its own `RUN` layer via `scripts/bake-models.sh`. Total image ~5.8 GB. |
