...
- **Call-mode relay (remote voice servers).** When the voice server advertises `relay_capable` in `/health` (any current miniclosedai-voice build), `/call/configure` sends `{relay: true}` instead of a callback URL. Per turn, the voice server emits `{turn_request: {turn_id, text}}` on its `/call/events` SSE; MiniClosedAI's events proxy intercepts it (never forwarded to the browser), runs the LLM turn locally (history + RAG + MCP + relay-route, `voice_mode` brevity), streams the reply as ndjson to the voice server's `POST /call/turn/{turn_id}` (per-sentence TTS starts on the first pushed sentence), and persists the (user, assistant) pair itself. Every connection is outbound from MiniClosedAI → RunPod/cloud voice servers work with zero config. Legacy voice servers keep the old direct `/chat/stream` callback (see `MINICLOSEDAI_PUBLIC_URL`).
...
| `MINICLOSEDAI_PUBLIC_URL` | unset | **Legacy voice servers only.** Public base URL of THIS server, handed to an old (pre-relay) remote voice backend as the call-mode callback. Relay-capable voice servers (any current build — `relay_capable` in `/health`) never need it: MiniClosedAI runs the LLM turn itself and pushes text to the voice server (`POST /call/turn/{id}`), all connections outbound. For legacy servers, unset = automatic: browser origin when public, else a locally-running ngrok tunnel is auto-discovered; if neither works, `/call/configure` returns 422 with this hint. |
| `MINICLOSEDAI_NGROK_API` | `http://127.0.0.1:4040` | Where the ngrok agent's local API lives, for the call-mode tunnel auto-discovery above. |
